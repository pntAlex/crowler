...
import { isBroken, isOrphan, referers, type Row } from "./crawler";
...
export const BROKEN_HEADER = [
  ...
  "target_kind", "total_referers", "referers_listed", "via_redirect",
];
 ...
 * Le referer est la page qui porte le lien, même quand celui-ci passe par des
 * redirections : `via_redirect` dit alors l'URL que la page lie (voir `referers`).
 * La colonne vient en dernier pour ne décaler aucune de celles qu'un tableur
 * exploite déjà.
 */
export async function* brokenRows(rows: Iterable<Row> | AsyncIterable<Row>, hops: ReadonlyMap<string, Row>) {
  for await (const r of rows) {
    if (!isBroken(r)) continue;
    const { refs, total } = referers(r, hops);
    const tail = [r.kind, total, refs.length];
    if (!refs.length) {
      yield [r.url, r.status || "", r.error ?? "", "", "", ...tail, ""];
      continue;
    }
    for (const ref of refs) {
      yield [r.url, r.status || "", r.error ?? "", ref.from, ref.text, ...tail, ref.via ?? ""];
    }
  }
}
